...
const VendorSignup: React.FC<VendorSignupProps> = ({ isOpen, onClose }) => {
  ...
  const [esewaNumber, setEsewaNumber] = useState<string>("");
  ...
  const validateField = (name: string, value: any): string => {
    switch (name) {
      ...
      case "blankChequePhoto":
        if (paymentMethodType !== "bank") return "";
      ...
      case "esewaNumber":
        if (!value || !value.trim()) return "eSewa phone number is required";
        if (!/^\d{10}$/.test(value.trim()))
          return "eSewa number must be exactly 10 digits";
        return "";

      case "khaltiNumber":
        if (!value || !value.trim()) return "Khalti phone number is required";
        if (!/^\d{10}$/.test(value.trim()))
          return "Khalti number must be exactly 10 digits";
        return "";
  ...
  const handleInputChange = (
    ...
    switch (name) {
      ...
      case "paymentMethodType":
        ...
        if (value !== "bank") {
          setBlankChequePhoto(null);
          setTouched((prev) => ({ ...prev, blankChequePhoto: false }));
          setErrors((prev) => {
            const nextErrors = { ...prev };
            delete nextErrors["blankChequePhoto"];
            return nextErrors;
          });
        }
        break;
      case "esewaNumber":
        setEsewaNumber(value);
        break;
      case "khaltiNumber":
            ...
            : paymentMethodType === "bank"
              ...
              : paymentMethodType === "esewa"
                ...
                : paymentMethodType === "khalti"
                  ...
                  : ["paymentMethodType", "acceptListingFee"];
  ...
  const handleFileChange = (
    ...
    if (documentType === "tax") {
      ...
      const file = files[0];
      if (file) {
        setBlankChequePhoto(file);
        setTouched((prev) => ({ ...prev, blankChequePhoto: true }));
        const error = validateField("blankChequePhoto", file);
  ...
  const handleSubmit = async (
    ...
    if (taxDocsError) {
      setErrors((prev) => ({ ...prev, taxDocuments: taxDocsError }));
      setTouched((prev) => ({ ...prev, taxDocuments: true }));
    ...
    const chequePhotoUrls =
      paymentMethodType === "bank" && blankChequePhoto
    ...
    if (paymentMethodType === "bank" && !chequePhotoUrls?.[0]) {
      setError("Failed to upload blank cheque photo. Please try again.");
      toast.error("Failed to upload blank cheque photo. Please try again.");
      setIsLoading(false);
      return;
    }
    ...
    const userData = {
      ...
      bankDetails:
        paymentMethodType === "bank"
          ...
          : {
              accountName: accountName.trim(),
              bankName: paymentMethodType === "esewa" ? "eSewa" : "Khalti",
              accountNumber:
                paymentMethodType === "esewa"
                  ? esewaNumber.trim()
                  : khaltiNumber.trim(),
          ...
          <form className="auth-modal__form" onSubmit={handleSubmit}>
            {showVerification ? (
              ...
              <>
                {currentStep === 1 && (
                  <>
                    ...
                    <div className=" auth-modal__checkbox">
                      ...
                      />
                      <label className="" style={{ background: "transparent" }}>
                        I accept the{" "}
                        <Link href="/vendor/terms" target="_blank">
                ...
                {currentStep === 2 && (
                  <>
                    <div className="auth-modal__step-section-header">
                      <h3 className="auth-modal__step-section-title">
                        Registration &amp; Security
                      </h3>
                      <p className="auth-modal__step-section-subtitle">
                    ...
                    <div className="auth-modal__form-group auth-modal__form-group--grid">
                      <div>
                        <label className="auth-modal__label">
                          ...
                          <FaInfoCircle
                            ...
                            title="Enter your official business registration number"
                          />
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="auth-modal__label">
                          Vat/Pan Number
                          <span className="auth-modal__required"> *</span>
                          <span className="auth-modal__label-note">
                            {" "}
                            (9 digits required)
                          </span>
                          <FaInfoCircle
                            className="auth-modal__info-icon"
                            title="Enter your 9-digit PAN or VAT number"
                          />
                        </label>
                        <input
                          type="text"
                    ...
                    <div className="auth-modal__form-group auth-modal__form-group--grid">
                      <div>
                        <label className="auth-modal__label">
                          Email <span className="auth-modal__required">*</span>
                        </label>
                        <input
                          type="email"
                          className={`auth-modal__input ${
                ...
                {currentStep === 3 && (
                  <>
                    ...
                    {/* Business & PAN/VAT Documents */}
                    <div className="auth-modal__form-group">
                      <div className="auth-modal__doc-label-row">
                        <span className="auth-modal__doc-label-title">
                          Business &amp; PAN/VAT Document(s)
                          <span className="auth-modal__required"> *</span>
                          <FaInfoCircle
                            className="auth-modal__info-icon"
                            title="Attach your business registration and PAN/VAT documents"
                          />
                        </span>
                        <span className="auth-modal__doc-label-hint">
                    ...
                    {/* Ownership Citizenship Documents */}
                    <div className="auth-modal__form-group">
                      <div className="auth-modal__doc-label-row">
                        <span className="auth-modal__doc-label-title">
                          ...
                          <FaInfoCircle
                            className="auth-modal__info-icon"
                            title="Optionally upload front and back of the owner's citizenship ID"
                          />
                        </span>
                        <span className="auth-modal__doc-label-hint">
                ...
                {currentStep === 4 && (
                  <>
                    ...
                    <div className="auth-modal__step-section-header">
                      ...
                      <p className="auth-modal__step-section-subtitle">
                        Add at least one payment method where you&apos;d like to
                    ...
                    <div className="auth-modal__form-group">
                      <div className="auth-modal__doc-label-row">
                        <span className="auth-modal__doc-label-title">
                          ...
                          <FaInfoCircle
                            className="auth-modal__info-icon"
                            title="Add at least one wallet or bank account"
                          />
                        </span>
                        <span className="auth-modal__doc-label-hint">
                          Add at least one wallet or bank account.
                        </span>
                      </div>

                      {/* Payment container */}
                      <div className="auth-modal__payment-container">
                        <label className="auth-modal__payment-type-label">
                          Choose Method Type
                        </label>
                        <select
                          className={`auth-modal__payment-select ${
                            errors["paymentMethodType"] &&
                            touched["paymentMethodType"]
                              ? "error"
                              : ""
                          }`}
                        ...
                        {/* eSewa fields */}
                        {paymentMethodType === "esewa" && (
                          <div className="auth-modal__payment-fields">
                            <div className="auth-modal__payment-info-banner">
                              <span className="auth-modal__payment-info-title">
                                Digital Wallet (Instant Settlement)
                              </span>
                              <span className="auth-modal__payment-info-desc">
                                Use this for fast, automated payments.
                                Recommended for local vendors with frequent
                                payouts.
                              </span>
                            </div>
                            <div className="auth-modal__form-group">
                            ...
                            <div className="auth-modal__form-group auth-modal__form-group--grid">
                              <div>
                                <label className="auth-modal__label">
                                  Wallet Number{" "}
                                  <span className="auth-modal__required">
                                    *
                                  </span>
                                </label>
                                <input
                                  type="text"
                                  className={`auth-modal__input ${errors["esewaNumber"] && touched["esewaNumber"] ? "error" : ""}`}
                          ...
                          <div className="auth-modal__payment-fields">
                            ...
                            <div className="auth-modal__form-group auth-modal__form-group--grid">
                              <div>
                                ...
                                <input
                                  type="text"
                                  className={`auth-modal__input ${errors["khaltiNumber"] && touched["khaltiNumber"] ? "error" : ""}`}
                                  placeholder="Enter wallet number"
                                  name="khaltiNumber"
                                  value={khaltiNumber}
                                  onChange={handleInputChange}
                        ...
                        {paymentMethodType === "bank" && (
                          <div className="auth-modal__payment-fields">
                            <div className="auth-modal__payment-info-banner">
                              ...
                              <span className="auth-modal__payment-info-desc">
                                Funds will be transferred directly to your bank
                                account. Suitable for larger, bulk settlements.
                              </span>
                            </div>
                            <div className="auth-modal__form-group auth-modal__form-group--grid">
                              <div>
                            ...
                            <div className="auth-modal__form-group">
                              <div className="auth-modal__doc-label-row">
                                <span className="auth-modal__doc-label-title">
                                  ...
                                  <FaInfoCircle
                                    className="auth-modal__info-icon"
                                    title="Attach a blank cheque for bank account verification"
                                  />
                                </span>
                                <span className="auth-modal__doc-label-hint">
                                  Required for bank settlement only. Accepted:
                                  JPG or PNG. Max 5MB.
                                </span>
                              </div>
                              <div className="auth-modal__file-upload">
                                <label
                                  htmlFor="blankChequePhoto"
                                  className="auth-modal__file-label"
                                >
                                  Choose File
                                </label>
                              ...
                              {blankChequePhoto && (
                                <div className="auth-modal__file-item">
                                  <span className="auth-modal__file-name">
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      setErrors((prev) => ({
                                        ...prev,
                                        blankChequePhoto: validateField(
                                          "blankChequePhoto",
                                          null,
                                        ),
                                      }));
                        ...
                        <button
                          type="button"
                          className="auth-modal__add-payment-btn"
